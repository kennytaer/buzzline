...
export async function loader(args: LoaderFunctionArgs) {
  ...
  try {
    ...
    // Only allow editing dynamic segments
    if (segment.type !== 'dynamic') {
      throw new Response("Only dynamic segments can be edited", { status: 400 });
    }
    ...
    console.error("Error loading segment edit:", error);
...
export async function action(args: ActionFunctionArgs) {
  ...
  if (actionType === "refresh") {
    try {
      ...
      const allContacts = await contactService.listContacts(orgId);
      const result = await contactListService.refreshSegmentLogic(orgId, segmentId, allContacts);
      
      if (!result) {
        return json({ error: "Failed to refresh segment or segment is not dynamic" }, { status: 400 });
      }

      return json({ 
        success: true, 
        refreshResult: result,
        message: `Segment refreshed! Changed from ${result.previousCount} to ${result.newCount} contacts.`
      });
    } catch (error) {
      console.error("Error refreshing segment:", error);
      return json({ error: "Failed to refresh segment" }, { status: 500 });
    }
  }
  ...
  if (actionType === "update") {
    ...
    try {
      ...
      // Update segment
      await contactListService.updateContactList(orgId, segmentId, {
        name: segmentName,
      ...
      const existingSegment = await contactListService.getContactList(orgId, segmentId);
      ...
      for (const contactId of previousContactIds) {
        if (!matchingContactIds.includes(contactId)) {
          ...
          if (contact) {
            contact.contactListIds = contact.contactListIds.filter((id: string) => id !== segmentId);
      ...
      return json({ 
        ...
        message: `Segment updated successfully! Now contains ${matchingContacts.length} contacts.`
      });
    } catch (error) {
...
// Filter evaluation logic (same as in new segment)
...
export default function EditSegment() {
  const { segment, availableFields, totalContacts } = useLoaderData<typeof loader>();
  ...
  const [filters, setFilters] = useState<FilterRule[]>(segment.filters || []);
            ...
            <div className="flex">
              ...
              <div className="ml-3">
                ...
                <div className="mt-2 text-sm text-green-700">
                  <p>{'message' in actionData ? actionData.message : 'Operation completed successfully'}</p>
                </div>
      ...
      <div className="max-w-4xl mx-auto">
        <div className="md:flex md:items-center md:justify-between mb-6">
          <div className="flex-1 min-w-0">
              ...
              <ol className="flex items-center space-x-4">
                ...
                <li>
                  <div className="flex items-center">
                    ...
                    <a href={`/dashboard/contacts/segments/${segment.id}`} className="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700">
                      {segment.name}
                    </a>
                  </div>
                </li>
            ...
            <h1 className="mt-2 text-2xl font-bold text-gray-900">Edit Segment: {segment.name}</h1>
          ...
          <div className="bg-white shadow rounded-lg p-6">
            ...
            <div className="flex space-x-3">
              ...
              <div className="text-sm text-gray-500 flex items-center">
                Current: {segment.contactCount || 0} contacts
                {segment.lastRefreshed && (
                  <span className="ml-2">
                    (Last refreshed: {new Date(segment.lastRefreshed).toLocaleDateString()})
                  </span>
                )}
              </div>
            </div>
